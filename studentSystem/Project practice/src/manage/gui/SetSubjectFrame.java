...
public class SetSubjectFrame extends JFrame {
	...
	private Vector colHead;
	    ...
	    //定义List
	    private List<SetSubject> sets;

	    //定义实体类
      SetSubject setSubject;
	    
	    //设置构造方法
 public SetSubjectFrame(String title) {
	 super(title);
	 Start();
 }
 //定义执行方法
  ...
  JLabel label1=new JLabel("科目编号: ");
  JLabel label2=new JLabel("科目名称: ");
  ...
  b2.addActionListener(new ActionListener() {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		setSubject.setSubject_id(Integer.parseInt(text1.getText()));
		setSubject.setSubject_name(text2.getText());
    int count=setService.insertsubject(setSubject);
...
if (count > 0) {
    ...
    JOptionPane.showMessageDialog(null, "科目编号只能唯一！", "警告", JOptionPane.WARNING_MESSAGE);
}
  ...
  table.addMouseListener(new MouseAdapter() {
	...
	private void fillTableData() {
        ...
        // 填充表记录
        rows.clear();//清空
        for (SetSubject setSubject : sets) {
            Vector<String> currentRow = new Vector<String>();
            currentRow.addElement(setSubject.getSubject_id()+"");
            currentRow.addElement(setSubject.getSubject_name());
            // 将当前行添加到记录行集
            rows.add(currentRow);
        }
