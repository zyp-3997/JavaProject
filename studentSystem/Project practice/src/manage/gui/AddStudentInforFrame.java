...
public class AddStudentInforFrame extends JFrame {
	...
	private Vector colHead;
	    ...
	    */
	    ...
	    //设置构造方法
 public AddStudentInforFrame(String title) {
	 super(title);
	 Start();
 }
 //定义执行方法
 ...
 // p4.setLayout(new GridLayout(1,4));
  ...
  frame.add(southJPanel,BorderLayout.NORTH);
...
int count=studentServiceImpl.addinfor(student);
Grade grade=new Grade();
grade.setStudent_id(Integer.parseInt(text1.getText()));
String result=findclassname(text2.getText());
grade.setClass_names(result);
grade.setStudent_names((text3.getText()));
grade.setEnglish(Integer.parseInt("0"));
grade.setChinese(Integer.parseInt("0"));
grade.setChemistry(Integer.parseInt("0"));
grade.setMath(Integer.parseInt("0"));
grade.setPhysics(Integer.parseInt("0"));
grade.setBiolog(Integer.parseInt("0"));
...
if (count > 0) {
    ...
    int count2=gradeServiceImpl.addgrade(grade);
    sets=studentServiceImpl.findinforAll();
    // 清空待删学生学号文本框
    text1.setText("");
    text2.setText("");
    ...
    JOptionPane.showMessageDialog(null, "学号只能唯一！", "警告", JOptionPane.WARNING_MESSAGE);
}
...
public String findclassname(String id){
    // 声明学生
String rString = null;
    // 获取数据库连接对象
    Connection conn = ConnectionManager.getConnection();
    // 定义SQL字符串
    String strSQL = "SELECT class_name FROM set_class WHERE class_id = ?";
    try {
        // 创建预备语句对象
        ...
        if (rs.next()) {
            // 创建学生实体
        	rString=rs.getString("class_name");
        }
